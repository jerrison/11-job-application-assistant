#!/usr/bin/env python3
"""Breezy HR application autofill."""
...
_FORM_READY_SELECTOR = "form, input[type='file'], input[type='text'], input[type='email'], textarea, select"
_APPLY_GATE_RE = re.compile(r"\bapply(?:\s+to\s+position)?\b", re.I)
...
_REVIEW_PATTERNS = (
    ...
    re.compile(r"\bcomplete application\b", re.I),
)
...
def _build_payload(out_dir: Path, provider: str) -> dict:
    ...
    return build_simple_payload(
        ...
        notes=["Breezy single-page form. Stop at review in draft mode."],
    )
...
def _breezy_form_visible(page) -> bool:
    return any(
        page.locator(selector).count()
        for selector in ("form", "input[type='file']", "input[type='text']", "input[type='email']", "textarea")
    )


def _advance_breezy_apply_gate(page) -> bool:
    if _breezy_form_visible(page):
        return False

    for role in ("button", "link"):
        apply_cta = page.get_by_role(role, name=_APPLY_GATE_RE).first
        if apply_cta.count():
            apply_cta.click()
            page.wait_for_selector(_FORM_READY_SELECTOR, timeout=15000)
            return True
    return False
...
def _run_browser(payload_path: Path, headless: bool, submit: bool) -> int:
    return run_simple_board_pipeline(
        ...
        post_navigate_hook=_advance_breezy_apply_gate,
    )
